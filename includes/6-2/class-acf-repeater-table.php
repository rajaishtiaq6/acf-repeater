...
class ACF_Repeater_Table {
		...
		<thead>
			<tr>
				...
				foreach ( $this->sub_fields as $sub_field ) :
					...
					$sub_field['id'] = '';
					?>
					<th <?php echo acf_esc_attrs( $attrs ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- ACF escaping function ?>>
	...
	public function row_handle( $i ) {
		...
		$title        = __( 'Drag to reorder', 'shaqi-acf-repeater' );
		$row_num_html = sprintf(
			'<span class="acf-row-number" title="%s">%d</span>',
			esc_html__( 'Click to reorder', 'shaqi-acf-repeater' ),
			$hr_row_num
		);
	...
	public function row_actions() {
		...
		<td class="acf-row-handle remove">
			<a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row" title="<?php esc_attr_e( 'Add row', 'shaqi-acf-repeater' ); ?>"></a>
			<a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#" data-event="duplicate-row" title="<?php esc_attr_e( 'Duplicate row', 'shaqi-acf-repeater' ); ?>"></a>
	...
	public function table_actions() {
		...
		<div class="acf-actions">
			<a class="acf-button acf-repeater-add-row button button-primary" href="#" data-event="add-row"><?php echo acf_esc_html( $this->field['button_label'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- ACF escaping function ?></a>
			<?php $this->pagination(); ?>
			<div class="clear"></div>
		</div>
		<?php
	...
	public function pagination() {
		...
		$html_current_page = sprintf(
			...
			'<label for="current-page-selector" class="screen-reader-text">' . __( 'Current Page', 'shaqi-acf-repeater' ) . '</label>',
			1,
			strlen( $total_pages )
		);
		...
		<div class="acf-tablenav tablenav-pages">
			<a class="first-page button acf-nav" aria-hidden="true" data-event="first-page" title="<?php esc_attr_e( 'First Page', 'shaqi-acf-repeater' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'First Page', 'shaqi-acf-repeater' ); ?></span>
				<span aria-hidden="true">&laquo;</span>
			</a>
			<a class="prev-page button acf-nav" aria-hidden="true" data-event="prev-page" title="<?php esc_attr_e( 'Previous Page', 'shaqi-acf-repeater' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Previous Page', 'shaqi-acf-repeater' ); ?></span>
				<span aria-hidden="true">&lsaquo;</span>
			</a>
			<span class="paging-input">
				<label for="current-page-selector" class="screen-reader-text"><?php esc_html_e( 'Current Page', 'shaqi-acf-repeater' ); ?></label>
				<span class="tablenav-paging-text" title="<?php esc_attr_e( 'Current Page', 'shaqi-acf-repeater' ); ?>">
				<?php
				printf(
					/* translators: 1: Current page, 2: Total pages. */
					esc_html_x( '%1$s of %2$s', 'paging', 'shaqi-acf-repeater' ),
			...
			<a class="next-page button acf-nav" data-event="next-page" title="<?php esc_attr_e( 'Next Page', 'shaqi-acf-repeater' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Next Page', 'shaqi-acf-repeater' ); ?></span>
				<span aria-hidden="true">&rsaquo;</span>
			</a>
			<a class="last-page button acf-nav" data-event="last-page" title="<?php esc_attr_e( 'Last Page', 'shaqi-acf-repeater' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Last Page', 'shaqi-acf-repeater' ); ?></span>
